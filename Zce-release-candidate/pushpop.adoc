...
The term POPRET refers to _c.popret, c.popretz, c.popret.l_ and _c.popretz.l_. The assembly syntax for all of these uses the mnemonics _c.popret/c.popretz_.

Common details for these instructions are in this section.

The _.l_ forms of the instructions allow a longer register list and also a larger additional stack adjustment (a three bit field in the encoding versus a 2 bit field). 
They are distinguished from the non _.l_ forms by the register list in the syntax.
For example:

[source,sail]
--
c.push {ra,s0-s2},   -32;# maps to c.push, with a limit of 4 registers in the list
c.push {ra,s0-s11}, -128;# maps to c.push.l which allows up to 13 registers and also a larger additional stack adjustment
--

The difference between _c.push_ and _c.pusha_ is whether argument registers are moved into saved registers. 
Again, the same mnemonic is used and the difference is in the argument list.
For example:

[source,sail]
--
c.push {ra,s0-s2},  {a0-a2},  -32;# maps to c.pusha, with a limit of 4 registers win the list
c.push {ra,s0-s11}, {a0-a3}, -128;# maps to c.pusha.l which allows up to 13 registers and also a larger additional stack adjustment
--
...
_c.pusha, c.pusha.l_ include _areg_list_. This may not match what was generated by the compiler.
...
or handles errors responses after completing the PUSH isntruction.
