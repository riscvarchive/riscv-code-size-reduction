<<<
[#insns-cm_lbu,reftext="cm.lbu: Load unsigned byte, 16-bit encoding"]
=== cm.lbu
...
[NOTE]
  If _uimm < 4_ the encoding is reserved for custom use, as the functionality overlaps with <<insns-c_lbu>>.
