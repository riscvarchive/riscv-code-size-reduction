<<<
[#insns-c_tbljalm,reftext="c.tbljalm: table jump and link to t0, 16-bit encoding"]
=== c.tbljalm

Synopsis::
table jump and link to t0, 16-bit encoding
...
{reg:[
    ...
    { bits:  8, name: 'index8', attr: ['index8<8'] },
...
[NOTE]
  ...
  For this encoding to decode as _TBLJALM_, _index8<8_ otherwise it's a _different_ encoding

[NOTE]

  M in the mnemonic refers to millicode, as t0 is used for calling millicode routines.
...
This instruction is used to dereference a table of PCs, and then jumps to the dereferenced PC and links to t0.
...
[#insn-c_tbljalm-SAIL,reftext="c.tbljalm SAIL code"]
Operation::
[source,sail]
--
...
//check for breakpoint trigger which takes an exception with timing=0
  ...
  if (debug_trigger(table_address) && MCONTROL.timing==1 && MCONTROL.action==1) {
    ...
    JALR t0, target_address[XLEN-1:0]&~0x1;
  }
}
--
